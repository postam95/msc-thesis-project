...
public class TakeMineralsHomeState : State
{
    // Provides only one instance of this class, thread-safely.
    private static TakeMineralsHomeState instance = null;
    private static readonly object padlock = new object();

    // Initiates the this state.
    TakeMineralsHomeState()
    {
    }

    // Handles the singleton object.
    public static TakeMineralsHomeState Instance
    {
        get
        {
            lock (padlock)
            {
                if (instance == null)
                {
                    instance = new TakeMineralsHomeState();
                }
                return instance;
            }
        }
